...
/// survive a restart.
final homeLanguageIdProvider = StateProvider.autoDispose<String?>(
  (ref) => null,
);
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  void _handleBack() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(AppLocalizations.of(context).homeExitConfirmation),
  ...
  Future<void> _pickLanguage(
            ...
            children: [
              Padding(
                ...
                child: Align(
                  ...
                  child: Text(
                    AppLocalizations.of(sheetContext).homeLanguagePickerTitle,
                    style: ProfileTypography.sectionTitle(sheetContext),
                  ),
                ),
              ),
              for (final lang in languages)
                ListTile(
                  title: Text(
                    lang.name,
                    style: ProfileTypography.body(sheetContext),
                  ),
    ...
    if (picked != null)
      ref.read(homeLanguageIdProvider.notifier).state = picked;
  }
      ...
      child: Scaffold(
        body: SafeArea(
          child: languagesAsync.when(
            ...
            error: (err, st) =>
                Center(child: Text(l10n.homeLanguagesLoadError(err))),
            data: (languages) {
              if (languages.isEmpty) {
                return Center(child: Text(l10n.homeNoCourses));
              }
              final selectedId = languages.any((l) => l.id == override)
                  ? override!
                  : languages.first.id;
              final selected = languages.firstWhere((l) => l.id == selectedId);
              final lessonsAsync = ref.watch(
                homeLessonsProvider((id: selected.id, name: selected.name)),
              );
                ...
                children: [
                      ...
                      children: [
                        InkWell(
                          ...
                          onTap: languages.length > 1
                              ? () => _pickLanguage(languages, selected.id)
                              : null,
                          child: Container(
                            height: 40,
                  ...
                  Expanded(
                    child: lessonsAsync.when(
                      ...
                      error: (err, st) =>
                          Center(child: Text(l10n.homeLessonsLoadError(err))),
                      data: (lessons) {
                        if (lessons.isEmpty) {
