...
final _legacyLessonProvider = FutureProvider.autoDispose.family<AdminLesson, String>((ref, lessonId) async {
  final lessons = await ref.watch(profileRepositoryProvider).fetchLegacyLessons();
  final match = lessons.where((l) => l.lessonId == lessonId).toList();
  final title = match.isNotEmpty ? match.first.title : lessonId;
  return ref.watch(legacyAdminRepositoryProvider).getContent(lessonId, title);
});
...
class AdminLessonEditScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(lesson.value?.title ?? 'Урок'),
        leading: IconButton(icon: const Icon(Icons.arrow_back), onPressed: () => context.go('/admin/courses/legacy')),
      ),
      body: lesson.when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (err, st) => Center(child: Text('Не удалось загрузить урок: $err')),
        data: (l) => ListView(
          ...
          children: [
            LessonEditorPanel(
              ...
              onSaveMaterial: (text) async {
                await repo.saveMaterialText(lessonId, l.title, text);
                reload();
              },
              ...
              onUploadMedia: (kind, bytes, filename) async {
                try {
                  await repo.uploadMedia(lessonId, l.title, kind: kind, bytes: bytes, filename: filename);
                  reload();
